...
import { committeeTypeOptions } from "@/lib/constants";
import {
  Form,
  FormControl,
  FormField,
...
const committeeFormSchema = z.object({
  type: z.enum([committeeTypeOptions[0].value, committeeTypeOptions[1].value], {
...
export default function AddCommitteeForm() {
      ...
      try {
        ...
        for (let i = 1; i < jsonData.length; i++) {
          ...
          if (typeValue && nameValue) {
            // Find matching type from options
            const matchedOption = committeeTypeOptions.find(
              (opt) => typeValue.includes(opt.label) || typeValue === opt.value
            );
            const type = matchedOption
              ? matchedOption.value
              : committeeTypeOptions[0].value;
        ...
        if (entries.length === 0) {
          setParseError(
            "कोई मान्य डेटा नहीं मिला। कृपया Excel फ़ाइल फ़ॉर्मैट जाँचें।"
          );
        } else {
        ...
        setParseError(
          "फ़ाइल पढ़ने में त्रुटि। कृपया सही Excel फ़ाइल अपलोड करें।"
        );
      }
    };
  ...
  const handleConfirmedSubmit = async (data) => {
    try {
      if (isEditing && committee) {
        ...
        navigate("/reports/entry/committee");
      } else {
      ...
      toast.error(
        isEditing ? "Failed to update committee" : "Failed to add committee",
        {
  ...
  const handleConfirmedBulkSubmit = async () => {
    ...
    try {
      ...
      toast.success(
        `${validEntries.length} समिति/संग्रहण केंद्र सफलतापूर्वक जोड़े गए`
      );
      clearFile();
    } catch (error) {
  ...
  return (
    <Card className="w-full max-w-4xl mx-auto">
      <CardHeader>
        ...
        <CardTitle>
          {isEditing ? "Edit Committee" : t("forms.committee.title")}
        </CardTitle>
        <CardDescription>
          {isEditing
          ...
          <TabsContent value="upload" className="space-y-6">
            {!uploadedFile ? (
              <div
                ...
                onDrop={handleDrop}
                onClick={() =>
                  document.getElementById("committee-excel-input").click()
                }
              >
                <input
