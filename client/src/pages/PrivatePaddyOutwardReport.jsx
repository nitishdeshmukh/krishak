...
import { usePrivatePaddyOutward } from '@/hooks/usePrivatePaddyOutward';

export default function PrivatePaddyOutwardReport() {
    const dispatch = useDispatch();
    const { pageIndex, pageSize } = useSelector(state => state.table);
    const { t } = useTranslation(['reports', 'common']);

    const { privatePaddyOutward, totalPages, currentPage, isLoading, isError, error, hasNext, hasPrev } = usePrivatePaddyOutward();
    ...
    if (isError) {
        return (
            <div className="flex flex-col items-center justify-center h-64 bg-card rounded-xl border">
                <p className="text-destructive mb-2 font-semibold">Error loading private paddy outward</p>
        ...
        return (
            <div className="flex items-center justify-center h-64 bg-card rounded-xl border">
                <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-muted-foreground text-sm">Loading private paddy outward records...</p>
                </div>
            </div>
        );
    }

    if (!isLoading && (!privatePaddyOutward || privatePaddyOutward.length === 0)) {
        return (
            <EmptyState
                title="No Private Paddy Outward Records Found"
                description="Add a new private paddy outward record to see it here"
                buttonText="Add Private Paddy Outward"
                addRoute="/outward/private-paddy"
            ...
            <Card className={"py-0"}>
                <CardContent className="p-6 overflow-hidden">
                    <div className="w-full overflow-x-auto">
                        <DataTable columns={columns} data={privatePaddyOutward || []} showFilters={true} />
                    </div>
                    <TablePagination
                        pageIndex={pageIndex}
