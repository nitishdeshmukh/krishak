...
import { useOtherSales } from '@/hooks/useOtherSales';

export default function OtherSalesDealReport() {
    const dispatch = useDispatch();
    const { pageIndex, pageSize } = useSelector(state => state.table);
    const { t } = useTranslation(['reports', 'common']);

    const { otherSales, totalPages, currentPage, isLoading, isError, error, hasNext, hasPrev } = useOtherSales();

    const columns = [
        {
            accessorKey: 'otherSaleNumber',
            header: 'सौदा नंबर',
            meta: { filterVariant: 'text' },
            cell: ({ row }) => (
                <div className="font-medium ">{row.getValue('otherSaleNumber') || '-'}</div>
            ),
        },
        {
            accessorKey: 'date',
    ...
    if (isError) {
        return (
            <div className="flex flex-col items-center justify-center h-64 bg-card rounded-xl border">
                <p className="text-destructive mb-2 font-semibold">Error loading other sales</p>
        ...
        return (
            <div className="flex items-center justify-center h-64 bg-card rounded-xl border">
                <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-muted-foreground text-sm">Loading other sales deals...</p>
                </div>
            </div>
        );
    }

    if (!isLoading && otherSales.length === 0) {
        return (
            <EmptyState
                title="No Other Sales Deals Found"
                description="Add a new other sales deal to see it here"
                buttonText="Add Other Sales"
                addRoute="/sales/other"
            ...
            <Card className={"py-0"}>
                <CardContent className="p-6 overflow-hidden">
                    <div className="w-full overflow-x-auto">
                        <DataTable columns={columns} data={otherSales} showFilters={true} />
                    </div>
                    <TablePagination
                        pageIndex={pageIndex}
