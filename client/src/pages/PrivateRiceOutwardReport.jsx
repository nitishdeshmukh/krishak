...
import { usePrivateRiceOutward } from '@/hooks/usePrivateRiceOutward';

export default function PrivateRiceOutwardReport() {
    const dispatch = useDispatch();
    const { pageIndex, pageSize } = useSelector(state => state.table);
    const { t } = useTranslation(['reports', 'common']);

    const { privateRiceOutward, totalPages, currentPage, isLoading, isError, error, hasNext, hasPrev } = usePrivateRiceOutward();
    ...
    if (isError) {
        return (
            <div className="flex flex-col items-center justify-center h-64 bg-card rounded-xl border">
                <p className="text-destructive mb-2 font-semibold">Error loading private rice outward</p>
        ...
        return (
            <div className="flex items-center justify-center h-64 bg-card rounded-xl border">
                <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-muted-foreground text-sm">Loading private rice outward records...</p>
                </div>
            </div>
        );
    }

    if (!isLoading && (!privateRiceOutward || privateRiceOutward.length === 0)) {
        return (
            <EmptyState
                title="No Private Rice Outward Records Found"
                description="Add a new private rice outward record to see it here"
                buttonText="Add Private Rice Outward"
                addRoute="/outward/private-rice"
            ...
            <Card className={"py-0"}>
                <CardContent className="p-6 overflow-hidden">
                    <div className="w-full overflow-x-auto">
                        <DataTable columns={columns} data={privateRiceOutward || []} showFilters={true} />
                    </div>
                    <TablePagination
                        pageIndex={pageIndex}
