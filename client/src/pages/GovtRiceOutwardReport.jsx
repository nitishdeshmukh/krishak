...
import { useGovtRiceOutward } from '@/hooks/useGovtRiceOutward';

export default function GovtRiceOutwardReport() {
    const dispatch = useDispatch();
    const { pageIndex, pageSize } = useSelector(state => state.table);
    const { t } = useTranslation(['reports', 'common']);

    const { govtRiceOutward, totalPages, currentPage, isLoading, isError, error, hasNext, hasPrev } = useGovtRiceOutward();
    ...
    if (isError) {
        return (
            <div className="flex flex-col items-center justify-center h-64 bg-card rounded-xl border">
                <p className="text-destructive mb-2 font-semibold">Error loading govt rice outward</p>
        ...
        return (
            <div className="flex items-center justify-center h-64 bg-card rounded-xl border">
                <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-muted-foreground text-sm">Loading govt rice outward records...</p>
                </div>
            </div>
        );
    }

    if (!isLoading && (!govtRiceOutward || govtRiceOutward.length === 0)) {
        return (
            <EmptyState
                title="No Govt Rice Outward Records Found"
                description="Add a new govt rice outward record to see it here"
                buttonText="Add Govt Rice Outward"
                addRoute="/outward/govt-rice"
            ...
            <Card className={"py-0"}>
                <CardContent className="p-6 overflow-hidden">
                    <div className="w-full overflow-x-auto">
                        <DataTable columns={columns} data={govtRiceOutward || []} showFilters={true} />
                    </div>
                    <TablePagination
                        pageIndex={pageIndex}
