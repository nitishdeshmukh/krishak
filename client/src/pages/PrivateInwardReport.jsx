...
export default function PrivateInwardReport() {
    ...
    const columns = [
        ...
        },
        {
            ...
            cell: ({ row }) => {
                ...
                return quantity ? (
                    <div className="text-sm font-semibold text-purple-600">{quantity} क्विंटल</div>
                ) : (
                    <span className="text-muted-foreground">-</span>
                );
            },
        ...
        return (
            <div className="flex items-center justify-center h-64 bg-card rounded-xl border">
                <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-muted-foreground text-sm">Loading private paddy inward...</p>
                </div>
            </div>
        );
    }

    // Empty state - no private paddy inward
    if (!isLoading && inwardEntries.length === 0) {
        return (
            <EmptyState
                title="आपने अभी तक कोई निजी धान आवक नहीं जोड़ी है!"
                description="नई निजी धान आवक जोड़ने के लिए नीचे दिए गए बटन पर क्लिक करें"
                buttonText="निजी धान आवक जोड़ें"
                addRoute="/inward/private-paddy"
