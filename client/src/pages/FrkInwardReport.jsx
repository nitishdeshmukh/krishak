...
import { useFrkInward } from '@/hooks/useFrkInward';

export default function FrkInwardReport() {
    const dispatch = useDispatch();
    const { pageIndex, pageSize } = useSelector(state => state.table);
    const { t } = useTranslation(['reports', 'common']);

    const { frkInward, totalPages, currentPage, isLoading, isError, error, hasNext, hasPrev } = useFrkInward();

    const columns = [
        {
            accessorKey: 'frkInwardNumber',
            header: 'FRK Inward No.',
            meta: { filterVariant: 'text' },
            cell: ({ row }) => (
                <div className="font-medium ">{row.getValue('frkInwardNumber') || row.original.inwardNumber || '-'}</div>
            ),
        },
        {
            accessorKey: 'date',
    ...
    if (isError) {
        return (
            <div className="flex flex-col items-center justify-center h-64 bg-card rounded-xl border">
                <p className="text-destructive mb-2 font-semibold">Error loading FRK inward</p>
        ...
        return (
            <div className="flex items-center justify-center h-64 bg-card rounded-xl border">
                <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-muted-foreground text-sm">Loading FRK inward records...</p>
                </div>
            </div>
        );
    }

    if (!isLoading && frkInward.length === 0) {
        return (
            <EmptyState
                title="No FRK Inward Records Found"
                description="Add a new FRK inward record to see it here"
                buttonText="Add FRK Inward"
                addRoute="/inward/frk"
            ...
            <Card className={"py-0"}>
                <CardContent className="p-6 overflow-hidden">
                    <div className="w-full overflow-x-auto">
                        <DataTable columns={columns} data={frkInward} showFilters={true} />
                    </div>
                    <TablePagination
                        pageIndex={pageIndex}
