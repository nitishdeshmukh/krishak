...
import { useHuskSales } from '@/hooks/useHuskSales';

export default function HuskSalesDealReport() {
    const dispatch = useDispatch();
    const { pageIndex, pageSize } = useSelector(state => state.table);
    const { t } = useTranslation(['reports', 'common']);

    const { huskSales, totalPages, currentPage, isLoading, isError, error, hasNext, hasPrev } = useHuskSales();

    const columns = [
        {
            accessorKey: 'huskSaleNumber',
            header: 'Husk Sale No.',
            meta: { filterVariant: 'text' },
            cell: ({ row }) => (
                <div className="font-medium ">{row.getValue('huskSaleNumber') || '-'}</div>
            ),
        },
        {
            accessorKey: 'date',
    ...
    if (isError) {
        return (
            <div className="flex flex-col items-center justify-center h-64 bg-card rounded-xl border">
                <p className="text-destructive mb-2 font-semibold">Error loading husk sales</p>
        ...
        return (
            <div className="flex items-center justify-center h-64 bg-card rounded-xl border">
                <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-muted-foreground text-sm">Loading husk sales deals...</p>
                </div>
            </div>
        );
    }

    if (!isLoading && huskSales.length === 0) {
        return (
            <EmptyState
                title="No Husk Sales Deals Found"
                description="Add a new husk sales deal to see it here"
                buttonText="Add Husk Sales"
                addRoute="/sales/husk"
            ...
            <Card className={"py-0"}>
                <CardContent className="p-6 overflow-hidden">
                    <div className="w-full overflow-x-auto">
                        <DataTable columns={columns} data={huskSales} showFilters={true} />
                    </div>
                    <TablePagination
                        pageIndex={pageIndex}
