...
import { usePrivateSackOutward } from '@/hooks/usePrivateSackOutward';

export default function PrivateSackOutwardReport() {
    const dispatch = useDispatch();
    const { pageIndex, pageSize } = useSelector(state => state.table);
    const { t } = useTranslation(['reports', 'common']);

    const { privateSackOutward, totalPages, currentPage, isLoading, isError, error, hasNext, hasPrev } = usePrivateSackOutward();
    ...
    if (isError) {
        return (
            <div className="flex flex-col items-center justify-center h-64 bg-card rounded-xl border">
                <p className="text-destructive mb-2 font-semibold">Error loading private sack outward</p>
        ...
        return (
            <div className="flex items-center justify-center h-64 bg-card rounded-xl border">
                <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-muted-foreground text-sm">Loading private sack outward records...</p>
                </div>
            </div>
        );
    }

    if (!isLoading && (!privateSackOutward || privateSackOutward.length === 0)) {
        return (
            <EmptyState
                title="No Private Sack Outward Records Found"
                description="Add a new private sack outward record to see it here"
                buttonText="Add Private Sack Outward"
                addRoute="/outward/private-sack"
            ...
            <Card className={"py-0"}>
                <CardContent className="p-6 overflow-hidden">
                    <div className="w-full overflow-x-auto">
                        <DataTable columns={columns} data={privateSackOutward || []} showFilters={true} />
                    </div>
                    <TablePagination
                        pageIndex={pageIndex}
