...
import { useSackInward } from '@/hooks/useSackInward';

export default function SackInwardReport() {
    const dispatch = useDispatch();
    const { pageIndex, pageSize } = useSelector(state => state.table);
    const { t } = useTranslation(['reports', 'common']);

    const { sackInward, totalPages, currentPage, isLoading, isError, error, hasNext, hasPrev } = useSackInward();

    const columns = [
        {
            accessorKey: 'sackInwardNumber',
            header: 'Sack Inward No.',
            meta: { filterVariant: 'text' },
            cell: ({ row }) => (
                <div className="font-medium ">{row.getValue('sackInwardNumber') || row.original.inwardNumber || '-'}</div>
            ),
        },
        {
            accessorKey: 'date',
    ...
    if (isError) {
        return (
            <div className="flex flex-col items-center justify-center h-64 bg-card rounded-xl border">
                <p className="text-destructive mb-2 font-semibold">Error loading sack inward</p>
        ...
        return (
            <div className="flex items-center justify-center h-64 bg-card rounded-xl border">
                <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-muted-foreground text-sm">Loading sack inward records...</p>
                </div>
            </div>
        );
    }

    if (!isLoading && sackInward.length === 0) {
        return (
            <EmptyState
                title="No Sack Inward Records Found"
                description="Add a new sack inward record to see it here"
                buttonText="Add Sack Inward"
                addRoute="/inward/sack"
            ...
            <Card className={"py-0"}>
                <CardContent className="p-6 overflow-hidden">
                    <div className="w-full overflow-x-auto">
                        <DataTable columns={columns} data={sackInward} showFilters={true} />
                    </div>
                    <TablePagination
                        pageIndex={pageIndex}
