...
import { useSackSales } from '@/hooks/useSackSales';

export default function SackSalesDealReport() {
    const dispatch = useDispatch();
    const { pageIndex, pageSize } = useSelector(state => state.table);
    const { t } = useTranslation(['reports', 'common']);

    const { sackSales, totalPages, currentPage, isLoading, isError, error, hasNext, hasPrev } = useSackSales();

    const columns = [
        {
            accessorKey: 'sackSaleNumber',
            header: 'Sack Sale No.',
            meta: { filterVariant: 'text' },
            cell: ({ row }) => (
                <div className="font-medium ">{row.getValue('sackSaleNumber') || '-'}</div>
            ),
        },
        {
            accessorKey: 'date',
    ...
    if (isError) {
        return (
            <div className="flex flex-col items-center justify-center h-64 bg-card rounded-xl border">
                <p className="text-destructive mb-2 font-semibold">Error loading sack sales</p>
        ...
        return (
            <div className="flex items-center justify-center h-64 bg-card rounded-xl border">
                <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-muted-foreground text-sm">Loading sack sales deals...</p>
                </div>
            </div>
        );
    }

    if (!isLoading && sackSales.length === 0) {
        return (
            <EmptyState
                title="No Sack Sales Deals Found"
                description="Add a new sack sales deal to see it here"
                buttonText="Add Sack Sales"
                addRoute="/sales/sack"
            ...
            <Card className={"py-0"}>
                <CardContent className="p-6 overflow-hidden">
                    <div className="w-full overflow-x-auto">
                        <DataTable columns={columns} data={sackSales} showFilters={true} />
                    </div>
                    <TablePagination
                        pageIndex={pageIndex}
