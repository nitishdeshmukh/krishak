...
import { useBrewersOutward } from '@/hooks/useBrewersOutward';

export default function BrewersOutwardReport() {
    const dispatch = useDispatch();
    const { pageIndex, pageSize } = useSelector(state => state.table);
    const { t } = useTranslation(['reports', 'common']);

    const { brewersOutward, totalPages, currentPage, isLoading, isError, error, hasNext, hasPrev } = useBrewersOutward();
    ...
    if (isError) {
        return (
            <div className="flex flex-col items-center justify-center h-64 bg-card rounded-xl border">
                <p className="text-destructive mb-2 font-semibold">Error loading brewers outward</p>
        ...
        return (
            <div className="flex items-center justify-center h-64 bg-card rounded-xl border">
                <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-muted-foreground text-sm">Loading brewers outward records...</p>
                </div>
            </div>
        );
    }

    if (!isLoading && (!brewersOutward || brewersOutward.length === 0)) {
        return (
            <EmptyState
                title="No Brewers Outward Records Found"
                description="Add a new brewers outward record to see it here"
                buttonText="Add Brewers Outward"
                addRoute="/outward/brewers"
            ...
            <Card className={"py-0"}>
                <CardContent className="p-6 overflow-hidden">
                    <div className="w-full overflow-x-auto">
                        <DataTable columns={columns} data={brewersOutward || []} showFilters={true} />
                    </div>
                    <TablePagination
                        pageIndex={pageIndex}
