...
import { usePaddySales, useDeletePaddySale } from "@/hooks/usePaddySales";
...
export default function PaddySalesDealReport() {
  ...
  const columns = [
...

    {
      ...
      meta: { filterVariant: "text" },
      cell: ({ row }) => {
        return <div className="text-sm">{row.getValue("salesType")}</div>;
      },
    },
    {
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return <div className="text-sm text-right">{value || "-"}</div>;
      },
    },
    {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      toast.success("धान बिक्री सौदा हटाया गया", {
          ...
          <div className="flex items-center justify-between px-6 py-1.5 border-b border-border/40">
            <div>
              <h2 className="text-lg font-semibold text-foreground">
                धान बिक्री सौदा विवरण
              </h2>
              <p className="text-sm text-muted-foreground">Paddy Sales Deal</p>
            </div>
            <DrawerClose asChild>
              <Button
                variant="ghost"
