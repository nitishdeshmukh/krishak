...
import { usePaddyMillSales, useDeletePaddySale } from "@/hooks/usePaddySales";
import { cn } from "@/lib/utils";
import { setPageIndex, setPageSize } from "@/store/slices/tableSlice";
import TablePagination from "@/components/ui/table-pagination";

export default function PaddyMillSalesDealReport() {
  ...
  // Fetch Mill paddy sales with pagination
  const {
    paddySales,
    totalPages,
    isLoading,
  ...
  // Table column definitions - NO DO Entries columns
  const columns = [
    {
      accessorKey: "dealNumber",
      header: "मिल बिक्री सौदा क्रमांक",
    ...
    },
    {
      ...
      meta: { filterVariant: "text" },
      cell: ({ row }) => (
        <div className="text-sm">{row.getValue("plasticGunnyRate") || "-"}</div>
      ),
    },
    {
      id: "actions",
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      toast.success("धान मिल बिक्री सौदा हटाया गया", {
    ...
    return (
      <div className="flex flex-col items-center justify-center h-64 bg-card rounded-xl border">
        <p className="text-destructive mb-2 font-semibold">
          Error loading paddy mill sales
        </p>
    ...
    return (
      <div className="flex items-center justify-center h-64 bg-card rounded-xl border">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-muted-foreground text-sm">
            Loading paddy mill sales deals...
          </p>
        </div>
      </div>
    );
  ...
  if (!isLoading && paddySales.length === 0) {
    return (
      <EmptyState
        title="आपने अभी तक कोई धान मिल बिक्री सौदा नहीं जोड़ा है!"
        description="नया धान मिल बिक्री सौदा जोड़ने के लिए नीचे दिए गए बटन पर क्लिक करें"
          ...
          <div className="flex items-center justify-between px-6 py-1.5 border-b border-border/40">
            <div>
              <h2 className="text-lg font-semibold text-foreground">
                धान मिल बिक्री सौदा विवरण
              </h2>
              <p className="text-sm text-muted-foreground">
                Paddy Mill Sales Deal
              </p>
            </div>
            <DrawerClose asChild>
              <Button
