...
import { useWhiteBranOutward } from '@/hooks/useWhiteBranOutward';

export default function WhiteBranOutwardReport() {
    const dispatch = useDispatch();
    const { pageIndex, pageSize } = useSelector(state => state.table);
    const { t } = useTranslation(['reports', 'common']);

    const { whiteBranOutward, totalPages, currentPage, isLoading, isError, error, hasNext, hasPrev } = useWhiteBranOutward();
    ...
    if (isError) {
        return (
            <div className="flex flex-col items-center justify-center h-64 bg-card rounded-xl border">
                <p className="text-destructive mb-2 font-semibold">Error loading white bran outward</p>
        ...
        return (
            <div className="flex items-center justify-center h-64 bg-card rounded-xl border">
                <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-muted-foreground text-sm">Loading white bran outward records...</p>
                </div>
            </div>
        );
    }

    if (!isLoading && (!whiteBranOutward || whiteBranOutward.length === 0)) {
        return (
            <EmptyState
                title="No White Bran Outward Records Found"
                description="Add a new white bran outward record to see it here"
                buttonText="Add White Bran Outward"
                addRoute="/outward/white-bran"
            ...
            <Card className={"py-0"}>
                <CardContent className="p-6 overflow-hidden">
                    <div className="w-full overflow-x-auto">
                        <DataTable columns={columns} data={whiteBranOutward || []} showFilters={true} />
                    </div>
                    <TablePagination
                        pageIndex={pageIndex}
