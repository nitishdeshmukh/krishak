...
import { useRiceSales } from '@/hooks/useRiceSales';

export default function RiceSalesDealReport() {
    const dispatch = useDispatch();
    const { pageIndex, pageSize } = useSelector(state => state.table);
    const { t } = useTranslation(['reports', 'common']);

    const { riceSales, totalPages, currentPage, isLoading, isError, error, hasNext, hasPrev } = useRiceSales();

    const columns = [
        {
            accessorKey: 'riceSaleNumber',
            header: 'Rice Sale No.',
            meta: { filterVariant: 'text' },
            cell: ({ row }) => (
                <div className="font-medium ">{row.getValue('riceSaleNumber') || '-'}</div>
            ),
        },
        {
            accessorKey: 'date',
    ...
    if (isError) {
        return (
            <div className="flex flex-col items-center justify-center h-64 bg-card rounded-xl border">
                <p className="text-destructive mb-2 font-semibold">Error loading rice sales</p>
        ...
        return (
            <div className="flex items-center justify-center h-64 bg-card rounded-xl border">
                <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-muted-foreground text-sm">Loading rice sales deals...</p>
                </div>
            </div>
        );
    }

    if (!isLoading && riceSales.length === 0) {
        return (
            <EmptyState
                title="No Rice Sales Deals Found"
                description="Add a new rice sales deal to see it here"
                buttonText="Add Rice Sales"
                addRoute="/sales/rice"
            ...
            <Card className={"py-0"}>
                <CardContent className="p-6 overflow-hidden">
                    <div className="w-full overflow-x-auto">
                        <DataTable columns={columns} data={riceSales} showFilters={true} />
                    </div>
                    <TablePagination
                        pageIndex={pageIndex}
