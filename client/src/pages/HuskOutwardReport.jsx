...
import { useHuskOutward } from '@/hooks/useHuskOutward';

export default function HuskOutwardReport() {
    const dispatch = useDispatch();
    const { pageIndex, pageSize } = useSelector(state => state.table);
    const { t } = useTranslation(['reports', 'common']);

    const { huskOutward, totalPages, currentPage, isLoading, isError, error, hasNext, hasPrev } = useHuskOutward();
    ...
    if (isError) {
        return (
            <div className="flex flex-col items-center justify-center h-64 bg-card rounded-xl border">
                <p className="text-destructive mb-2 font-semibold">Error loading husk outward</p>
        ...
        return (
            <div className="flex items-center justify-center h-64 bg-card rounded-xl border">
                <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-muted-foreground text-sm">Loading husk outward records...</p>
                </div>
            </div>
        );
    }

    if (!isLoading && (!huskOutward || huskOutward.length === 0)) {
        return (
            <EmptyState
                title="No Husk Outward Records Found"
                description="Add a new husk outward record to see it here"
                buttonText="Add Husk Outward"
                addRoute="/outward/husk"
            ...
            <Card className={"py-0"}>
                <CardContent className="p-6 overflow-hidden">
                    <div className="w-full overflow-x-auto">
                        <DataTable columns={columns} data={huskOutward || []} showFilters={true} />
                    </div>
                    <TablePagination
                        pageIndex={pageIndex}
