...
import { useBrokensOutward } from '@/hooks/useBrokensOutward';

export default function BrokensOutwardReport() {
    const dispatch = useDispatch();
    const { pageIndex, pageSize } = useSelector(state => state.table);
    const { t } = useTranslation(['reports', 'common']);

    const { brokensOutward, totalPages, currentPage, isLoading, isError, error, hasNext, hasPrev } = useBrokensOutward();
    ...
    if (isError) {
        return (
            <div className="flex flex-col items-center justify-center h-64 bg-card rounded-xl border">
                <p className="text-destructive mb-2 font-semibold">Error loading brokens outward</p>
        ...
        return (
            <div className="flex items-center justify-center h-64 bg-card rounded-xl border">
                <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-muted-foreground text-sm">Loading brokens outward records...</p>
                </div>
            </div>
        );
    }

    if (!isLoading && (!brokensOutward || brokensOutward.length === 0)) {
        return (
            <EmptyState
                title="No Brokens Outward Records Found"
                description="Add a new brokens outward record to see it here"
                buttonText="Add Brokens Outward"
                addRoute="/outward/brokens"
            ...
            <Card className={"py-0"}>
                <CardContent className="p-6 overflow-hidden">
                    <div className="w-full overflow-x-auto">
                        <DataTable columns={columns} data={brokensOutward || []} showFilters={true} />
                    </div>
                    <TablePagination
                        pageIndex={pageIndex}
