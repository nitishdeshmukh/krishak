...
import { useRiceBranOutward } from '@/hooks/useRiceBranOutward';

export default function RiceBranOutwardReport() {
    const dispatch = useDispatch();
    const { pageIndex, pageSize } = useSelector(state => state.table);
    const { t } = useTranslation(['reports', 'common']);

    const { riceBranOutward, totalPages, currentPage, isLoading, isError, error, hasNext, hasPrev } = useRiceBranOutward();
    ...
    if (isError) {
        return (
            <div className="flex flex-col items-center justify-center h-64 bg-card rounded-xl border">
                <p className="text-destructive mb-2 font-semibold">Error loading rice bran outward</p>
        ...
        return (
            <div className="flex items-center justify-center h-64 bg-card rounded-xl border">
                <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-muted-foreground text-sm">Loading rice bran outward records...</p>
                </div>
            </div>
        );
    }

    if (!isLoading && (!riceBranOutward || riceBranOutward.length === 0)) {
        return (
            <EmptyState
                title="No Rice Bran Outward Records Found"
                description="Add a new rice bran outward record to see it here"
                buttonText="Add Rice Bran Outward"
                addRoute="/outward/rice-bran"
            ...
            <Card className={"py-0"}>
                <CardContent className="p-6 overflow-hidden">
                    <div className="w-full overflow-x-auto">
                        <DataTable columns={columns} data={riceBranOutward || []} showFilters={true} />
                    </div>
                    <TablePagination
                        pageIndex={pageIndex}
