...
import { useFrkOutward } from '@/hooks/useFrkOutward';

export default function FrkOutwardReport() {
    const dispatch = useDispatch();
    const { pageIndex, pageSize } = useSelector(state => state.table);
    const { t } = useTranslation(['reports', 'common']);

    const { frkOutward, totalPages, currentPage, isLoading, isError, error, hasNext, hasPrev } = useFrkOutward();
    ...
    if (isError) {
        return (
            <div className="flex flex-col items-center justify-center h-64 bg-card rounded-xl border">
                <p className="text-destructive mb-2 font-semibold">Error loading FRK outward</p>
        ...
        return (
            <div className="flex items-center justify-center h-64 bg-card rounded-xl border">
                <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-muted-foreground text-sm">Loading FRK outward records...</p>
                </div>
            </div>
        );
    }

    if (!isLoading && (!frkOutward || frkOutward.length === 0)) {
        return (
            <EmptyState
                title="No FRK Outward Records Found"
                description="Add a new FRK outward record to see it here"
                buttonText="Add FRK Outward"
                addRoute="/outward/frk"
            ...
            <Card className={"py-0"}>
                <CardContent className="p-6 overflow-hidden">
                    <div className="w-full overflow-x-auto">
                        <DataTable columns={columns} data={frkOutward || []} showFilters={true} />
                    </div>
                    <TablePagination
                        pageIndex={pageIndex}
