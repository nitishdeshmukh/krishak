...
import { useRiceBranSales } from '@/hooks/useRiceBranSales';

export default function RiceBranSalesDealReport() {
    const dispatch = useDispatch();
    const { pageIndex, pageSize } = useSelector(state => state.table);
    const { t } = useTranslation(['reports', 'common']);

    const { riceBranSales, totalPages, currentPage, isLoading, isError, error, hasNext, hasPrev } = useRiceBranSales();

    const columns = [
        {
            accessorKey: 'riceBranSaleNumber',
            header: 'Rice Bran Sale No.',
            meta: { filterVariant: 'text' },
            cell: ({ row }) => (
                <div className="font-medium ">{row.getValue('riceBranSaleNumber') || '-'}</div>
            ),
        },
        {
            accessorKey: 'date',
    ...
    if (isError) {
        return (
            <div className="flex flex-col items-center justify-center h-64 bg-card rounded-xl border">
                <p className="text-destructive mb-2 font-semibold">Error loading rice bran sales</p>
        ...
        return (
            <div className="flex items-center justify-center h-64 bg-card rounded-xl border">
                <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-muted-foreground text-sm">Loading rice bran sales deals...</p>
                </div>
            </div>
        );
    }

    if (!isLoading && riceBranSales.length === 0) {
        return (
            <EmptyState
                title="No Rice Bran Sales Deals Found"
                description="Add a new rice bran sales deal to see it here"
                buttonText="Add Rice Bran Sales"
                addRoute="/sales/rice-bran"
            ...
            <Card className={"py-0"}>
                <CardContent className="p-6 overflow-hidden">
                    <div className="w-full overflow-x-auto">
                        <DataTable columns={columns} data={riceBranSales} showFilters={true} />
                    </div>
                    <TablePagination
                        pageIndex={pageIndex}
