...
import { useFRKSales } from '@/hooks/useFRKSales';

export default function FRKSalesDealReport() {
    const dispatch = useDispatch();
    const { pageIndex, pageSize } = useSelector(state => state.table);
    const { t } = useTranslation(['reports', 'common']);

    const { frkSales, totalPages, currentPage, isLoading, isError, error, hasNext, hasPrev } = useFRKSales();

    const columns = [
        {
            accessorKey: 'frkSaleNumber',
            header: 'FRK Sale No.',
            meta: { filterVariant: 'text' },
            cell: ({ row }) => (
                <div className="font-medium ">{row.getValue('frkSaleNumber') || '-'}</div>
            ),
        },
        {
            accessorKey: 'date',
    ...
    if (isError) {
        return (
            <div className="flex flex-col items-center justify-center h-64 bg-card rounded-xl border">
                <p className="text-destructive mb-2 font-semibold">Error loading FRK sales</p>
        ...
        return (
            <div className="flex items-center justify-center h-64 bg-card rounded-xl border">
                <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-muted-foreground text-sm">Loading FRK sales deals...</p>
                </div>
            </div>
        );
    }

    if (!isLoading && frkSales.length === 0) {
        return (
            <EmptyState
                title="No FRK Sales Deals Found"
                description="Add a new FRK sales deal to see it here"
                buttonText="Add FRK Sales"
                addRoute="/sales/frk"
            ...
            <Card className={"py-0"}>
                <CardContent className="p-6 overflow-hidden">
                    <div className="w-full overflow-x-auto">
                        <DataTable columns={columns} data={frkSales} showFilters={true} />
                    </div>
                    <TablePagination
                        pageIndex={pageIndex}
