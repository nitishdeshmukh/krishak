...
import { useWhiteBranSales } from '@/hooks/useWhiteBranSales';

export default function WhiteBranSalesDealReport() {
    const dispatch = useDispatch();
    const { pageIndex, pageSize } = useSelector(state => state.table);
    const { t } = useTranslation(['reports', 'common']);

    const { whiteBranSales, totalPages, currentPage, isLoading, isError, error, hasNext, hasPrev } = useWhiteBranSales();

    const columns = [
        {
            accessorKey: 'whiteBranSaleNumber',
            header: 'White Bran Sale No.',
            meta: { filterVariant: 'text' },
            cell: ({ row }) => (
                <div className="font-medium ">{row.getValue('whiteBranSaleNumber') || '-'}</div>
            ),
        },
        {
            accessorKey: 'date',
    ...
    if (isError) {
        return (
            <div className="flex flex-col items-center justify-center h-64 bg-card rounded-xl border">
                <p className="text-destructive mb-2 font-semibold">Error loading white bran sales</p>
        ...
        return (
            <div className="flex items-center justify-center h-64 bg-card rounded-xl border">
                <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-muted-foreground text-sm">Loading white bran sales deals...</p>
                </div>
            </div>
        );
    }

    if (!isLoading && whiteBranSales.length === 0) {
        return (
            <EmptyState
                title="No White Bran Sales Deals Found"
                description="Add a new white bran sales deal to see it here"
                buttonText="Add White Bran Sales"
                addRoute="/sales/white-bran"
            ...
            <Card className={"py-0"}>
                <CardContent className="p-6 overflow-hidden">
                    <div className="w-full overflow-x-auto">
                        <DataTable columns={columns} data={whiteBranSales} showFilters={true} />
                    </div>
                    <TablePagination
                        pageIndex={pageIndex}
