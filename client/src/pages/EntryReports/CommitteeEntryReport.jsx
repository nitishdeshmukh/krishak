...
import { ArrowPathIcon, EllipsisHorizontalIcon, EyeIcon, PencilIcon, TrashIcon, BuildingOffice2Icon, CalendarIcon } from '@heroicons/react/24/outline';
        ...
        return (
            <div className="flex items-center justify-center h-64 bg-card rounded-xl border">
                <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-muted-foreground text-sm">Loading committee information...</p>
                </div>
            </div>
        );
    }
    ...
    if (isError) {
        return (
            <div className="flex flex-col items-center justify-center h-64 bg-card rounded-xl border">
                <p className="text-destructive mb-2 font-semibold">Error loading committees</p>
            ...
            <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            Are you sure you want to delete <span className="font-semibold text-foreground">{committeeToDelete?.committeeName}</span>? 
            ...
            <Drawer open={isDrawerOpen} onOpenChange={setIsDrawerOpen} direction="right">
                <DrawerContent className="h-full w-full sm:max-w-[420px] border-l shadow-2xl">
                    ...
                    <div className="flex items-center justify-between px-6 py-1.5 border-b border-border/40">
                        <div>
                            <h2 className="text-lg font-semibold text-foreground">Committee Details</h2>
                            ...
                            <div className="p-4 rounded-2xl border border-border/50 bg-card/50">
                                <div className="flex items-start justify-between gap-3">
                                    <div className="flex-1 min-w-0">
                                        <p className="text-xs text-muted-foreground mb-1">Committee Name</p>
                                        <p className="text-base font-semibold text-foreground truncate">
                                            {selectedCommittee.committeeName || <span className="text-muted-foreground/50">Not provided</span>}
                                        </p>
                                    </div>
                                    <span className={cn(
                                        ...
                                        <span className={cn(
                                            ...
                                            selectedCommittee.isActive !== false ? "bg-success" : "bg-destructive"
                                        )} />
                                        {selectedCommittee.isActive !== false ? 'Active' : 'Inactive'}
                                    </span>
                                </div>
                            </div>
                                ...
                                <div className="flex-1 rounded-xl border border-border/50 bg-muted/20 px-4 py-3 text-center">
                                    ...
                                    <p className="text-xs font-medium text-foreground">
                                        {selectedCommittee.createdAt 
                                            ? new Date(selectedCommittee.createdAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })
                                            : '—'
                                        }
                                    </p>
                                </div>
                                <div className="flex-1 rounded-xl border border-border/50 bg-muted/20 px-4 py-3 text-center">
                                    ...
                                    <p className="text-xs font-medium text-foreground">
                                        {selectedCommittee.updatedAt 
                                            ? new Date(selectedCommittee.updatedAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })
                                            : '—'
                                        }
                                    </p>
                                </div>
