...
import { useBrokensSales } from '@/hooks/useBrokensSales';

export default function BrokensSalesDealReport() {
    const dispatch = useDispatch();
    const { pageIndex, pageSize } = useSelector(state => state.table);
    const { t } = useTranslation(['reports', 'common']);

    const { brokensSales, totalPages, currentPage, isLoading, isError, error, hasNext, hasPrev } = useBrokensSales();

    const columns = [
        {
            accessorKey: 'brokensSaleNumber',
            header: 'Brokens Sale No.',
            meta: { filterVariant: 'text' },
            cell: ({ row }) => (
                <div className="font-medium ">{row.getValue('brokensSaleNumber') || '-'}</div>
            ),
        },
        {
            accessorKey: 'date',
    ...
    if (isError) {
        return (
            <div className="flex flex-col items-center justify-center h-64 bg-card rounded-xl border">
                <p className="text-destructive mb-2 font-semibold">Error loading brokens sales</p>
        ...
        return (
            <div className="flex items-center justify-center h-64 bg-card rounded-xl border">
                <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-muted-foreground text-sm">Loading brokens sales deals...</p>
                </div>
            </div>
        );
    }

    if (!isLoading && brokensSales.length === 0) {
        return (
            <EmptyState
                title="No Brokens Sales Deals Found"
                description="Add a new brokens sales deal to see it here"
                buttonText="Add Brokens Sales"
                addRoute="/sales/brokens"
            ...
            <Card className={"py-0"}>
                <CardContent className="p-6 overflow-hidden">
                    <div className="w-full overflow-x-auto">
                        <DataTable columns={columns} data={brokensSales} showFilters={true} />
                    </div>
                    <TablePagination
                        pageIndex={pageIndex}
