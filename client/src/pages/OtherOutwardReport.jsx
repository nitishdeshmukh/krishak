...
import { useOtherOutward } from '@/hooks/useOtherOutward';

export default function OtherOutwardReport() {
    const dispatch = useDispatch();
    const { pageIndex, pageSize } = useSelector(state => state.table);
    const { t } = useTranslation(['reports', 'common']);

    // Use hook for data
    const { otherOutward, totalPages, currentPage, isLoading, isError, error, hasNext, hasPrev } = useOtherOutward();
    ...
    if (isError) {
        return (
            <div className="flex flex-col items-center justify-center h-64 bg-card rounded-xl border">
                <p className="text-destructive mb-2 font-semibold">Error loading other outward</p>
        ...
        return (
            <div className="flex items-center justify-center h-64 bg-card rounded-xl border">
                <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-muted-foreground text-sm">Loading other outward records...</p>
                </div>
            </div>
        );
    }

    if (!isLoading && (!otherOutward || otherOutward.length === 0)) {
        return (
            <EmptyState
                title="No Other Outward Records Found"
                description="Add a new other outward record to see it here"
                buttonText="Add Other Outward"
                addRoute="/outward/other"
            ...
            <Card className={"py-0"}>
                <CardContent className="p-6 overflow-hidden">
                    <div className="w-full overflow-x-auto">
                        <DataTable columns={columns} data={otherOutward || []} showFilters={true} />
                    </div>
                    <TablePagination
                        pageIndex={pageIndex}
