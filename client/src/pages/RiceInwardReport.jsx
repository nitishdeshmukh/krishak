...
export default function RiceInwardReport() {
    ...
    const columns = [
        ...
        },
        {
            ...
            cell: ({ row }) => {
                ...
                return quantity ? (
                    <div className="text-sm font-semibold text-amber-600">{quantity} क्विंटल</div>
                ) : (
                    <span className="text-muted-foreground">-</span>
                );
            },
        ...
        return (
            <div className="flex items-center justify-center h-64 bg-card rounded-xl border">
                <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-muted-foreground text-sm">Loading rice inward / lot deposit...</p>
                </div>
            </div>
        );
    }

    // Empty state - no rice inward
    if (!isLoading && inwardEntries.length === 0) {
        return (
            <EmptyState
                title="आपने अभी तक कोई चावल आवक / LOT जमा नहीं जोड़ा है!"
                description="नया चावल आवक / LOT जमा जोड़ने के लिए नीचे दिए गए बटन पर क्लिक करें"
                buttonText="चावल आवक जोड़ें"
                addRoute="/inward/rice"
