...
import { useOtherInward } from '@/hooks/useOtherInward';

export default function OtherInwardReport() {
    const dispatch = useDispatch();
    const { pageIndex, pageSize } = useSelector(state => state.table);
    const { t } = useTranslation(['reports', 'common']);

    // Use hook for data
    const { otherInward, totalPages, currentPage, isLoading, isError, error, hasNext, hasPrev } = useOtherInward();
    ...
    if (isError) {
        return (
            <div className="flex flex-col items-center justify-center h-64 bg-card rounded-xl border">
                <p className="text-destructive mb-2 font-semibold">Error loading other inward</p>
        ...
        return (
            <div className="flex items-center justify-center h-64 bg-card rounded-xl border">
                <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-muted-foreground text-sm">Loading other inward records...</p>
                </div>
            </div>
        );
    }

    if (!isLoading && (!otherInward || otherInward.length === 0)) {
        return (
            <EmptyState
                title="No Other Inward Records Found"
                description="Add a new other inward record to see it here"
                buttonText="Add Other Inward"
                addRoute="/inward/other"
            ...
            <Card className={"py-0"}>
                <CardContent className="p-6 overflow-hidden">
                    <div className="w-full overflow-x-auto">
                        <DataTable columns={columns} data={otherInward || []} showFilters={true} />
                    </div>
                    <TablePagination
                        pageIndex={pageIndex}
