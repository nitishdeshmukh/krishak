...
import { useFRKPurchases } from '@/hooks/useFRKPurchases';
...
export default function FRKPurchaseDealReport() {
    ...
    const { frkPurchases, totalPages, currentPage, isLoading, isError, error, hasNext, hasPrev } = useFRKPurchases();

    const columns = [
        {
            accessorKey: 'frkPurchaseNumber',
            header: 'FRK Purchase No.',
            meta: { filterVariant: 'text' },
            cell: ({ row }) => (
                <div className="font-medium ">{row.getValue('frkPurchaseNumber') || '-'}</div>
            ),
        },
        {
            accessorKey: 'date',
    ...
    if (isError) {
        return (
            <div className="flex flex-col items-center justify-center h-64 bg-card rounded-xl border">
                <p className="text-destructive mb-2 font-semibold">Error loading FRK purchases</p>
        ...
        return (
            <div className="flex items-center justify-center h-64 bg-card rounded-xl border">
                <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-muted-foreground text-sm">Loading FRK purchase deals...</p>
                </div>
            </div>
        );
    }
            ...
            <Card className={"py-0"}>
                <CardContent className="p-6 overflow-hidden">
                    <div className="w-full overflow-x-auto">
                        <DataTable columns={columns} data={frkPurchases} showFilters={true} />
                    </div>
                    <TablePagination
                        pageIndex={pageIndex}
