...
export const useCreatePaddySale = () => {
  ...
  return useMutation({
    ...
    onSuccess: () =>
      queryClient.invalidateQueries({ queryKey: ["paddySales"] }),
  });
};
...
export const useDeletePaddySale = () => {
  ...
  return useMutation({
    mutationFn: (id) =>
      import("../api/paddySalesApi").then((module) =>
        module.deletePaddySale(id)
      ),
    onSuccess: () =>
      queryClient.invalidateQueries({ queryKey: ["paddySales"] }),
  });
